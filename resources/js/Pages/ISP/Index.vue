      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
          <div class="flex justify-between mb-6">
            ...
            <Link :href="route('isp.create')" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
              Add ISP
            </Link>
          </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="isp in isps.data" :key="isp.id">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <Link :href="route('isp.show', isp.id)" class="text-indigo-600 hover:text-indigo-900 mr-3">View</Link>
                  <Link :href="route('isp.edit', isp.id)" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</Link>
  ...
  methods: {
    deleteISP(isp) {
      if (confirm('Are you sure you want to delete this ISP?')) {
        this.$inertia.delete(route('isp.destroy', isp.id))
      }
    }
  }
}
